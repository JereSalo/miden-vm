...
use miden_mast_package::debug_info::{
    DebugFieldInfo, DebugInfoBuilder, DebugPrimitiveType, DebugStringIdx, DebugTypeIdx,
    DebugTypeInfo, DebugVariantInfo,
};

use super::{MastNodeRef, SourceNodeRef};
...
pub fn register_debug_type(
    debug_info: &mut DebugInfoBuilder<MastNodeRef, SourceNodeRef>,
    declared_name: Option<DebugStringIdx>,
    declared_ty: Option<&TypeExpr>,
    ty: &Type,
) -> Result<DebugTypeIdx, Report> {
    Ok(match ty {
        Type::I1 => debug_info.add_type(DebugTypeInfo::Primitive(DebugPrimitiveType::Bool)),
        Type::I8 => debug_info.add_type(DebugTypeInfo::Primitive(DebugPrimitiveType::I8)),
        Type::U8 => debug_info.add_type(DebugTypeInfo::Primitive(DebugPrimitiveType::U8)),
        Type::I16 => debug_info.add_type(DebugTypeInfo::Primitive(DebugPrimitiveType::I16)),
        Type::U16 => debug_info.add_type(DebugTypeInfo::Primitive(DebugPrimitiveType::U16)),
        Type::I32 => debug_info.add_type(DebugTypeInfo::Primitive(DebugPrimitiveType::I32)),
        Type::U32 => debug_info.add_type(DebugTypeInfo::Primitive(DebugPrimitiveType::U32)),
        Type::I64 => debug_info.add_type(DebugTypeInfo::Primitive(DebugPrimitiveType::I64)),
        Type::U64 => debug_info.add_type(DebugTypeInfo::Primitive(DebugPrimitiveType::U64)),
        Type::I128 => debug_info.add_type(DebugTypeInfo::Primitive(DebugPrimitiveType::I128)),
        Type::U128 => debug_info.add_type(DebugTypeInfo::Primitive(DebugPrimitiveType::U128)),
        Type::Felt => debug_info.add_type(DebugTypeInfo::Primitive(DebugPrimitiveType::Felt)),
        Type::F64 => debug_info.add_type(DebugTypeInfo::Primitive(DebugPrimitiveType::F64)),
        Type::U256 => debug_info.add_type(DebugTypeInfo::Primitive(DebugPrimitiveType::U256)),
        Type::Unknown => debug_info.add_type(DebugTypeInfo::Unknown),
        Type::Never => debug_info.add_type(DebugTypeInfo::Primitive(DebugPrimitiveType::Void)),
        Type::Ptr(ptr) => {
            ...
            let pointee_name = pointee_name.map(|name| debug_info.add_string(name));
            let pointee_decl = declared_ty.and_then(|t| match t {
                TypeExpr::Ptr(p) => Some(p.pointee.as_ref()),
                _ => None,
            });
            let pointee_type_idx =
                register_debug_type(debug_info, pointee_name, pointee_decl, ptr.pointee())?;
            debug_info.add_type(DebugTypeInfo::Pointer { pointee_type_idx })
        },
        Type::Array(array) => {
            ...
            let element_name = element_name.map(|name| debug_info.add_string(name));
            let element_decl = declared_ty.and_then(|t| match t {
                TypeExpr::Ptr(p) => Some(p.pointee.as_ref()),
                _ => None,
            });
            let element_type_idx =
                register_debug_type(debug_info, element_name, element_decl, array.element_type())?;
            let count =
                u32::try_from(array.len()).map_err(|_| Report::msg("array type is too large"))?;
            debug_info.add_type(DebugTypeInfo::Array { element_type_idx, count: Some(count) })
        },
        Type::List(ty) => {
            let pointee_name = declared_ty.and_then(|t| match t {
                TypeExpr::Ptr(p) => match p.pointee.as_ref() {
            ...
            let pointee_ty = register_debug_type(debug_info, pointee_name, pointee_decl, ty)?;
            let usize_ty = debug_info.add_type(DebugTypeInfo::Primitive(DebugPrimitiveType::U32));
            let pointer_ty =
                debug_info.add_type(DebugTypeInfo::Pointer { pointee_type_idx: pointee_ty });
            let name_idx =
                declared_name.unwrap_or_else(|| debug_info.add_string(format!("list<{ty}>")));
            let ptr = DebugFieldInfo {
                name_idx: debug_info.add_string("ptr"),
                type_idx: pointer_ty,
                offset: 0,
            };
            let len = DebugFieldInfo {
                name_idx: debug_info.add_string("len"),
                type_idx: usize_ty,
                offset: 4,
            };
            debug_info.add_type(DebugTypeInfo::Struct {
                name_idx,
                size: 8,
                fields: vec![ptr, len],
            })
        ...
        Type::Struct(struct_ty) => {
            ...
            for (i, field) in struct_ty.fields().iter().enumerate() {
                ...
                let field_type_name = field_type_name.map(|name| debug_info.add_string(name));
                let type_idx =
                    register_debug_type(debug_info, field_type_name, declared_ty, &field.ty)?;
                let name_idx =
                    debug_info.add_string(field_name.unwrap_or_else(|| format!("{i}").into()));
                fields.push(DebugFieldInfo { name_idx, type_idx, offset: field.offset });
            }
            let struct_name =
                declared_name.or_else(|| struct_ty.name().map(|name| debug_info.add_string(name)));
            let name_idx = struct_name.unwrap_or_else(|| debug_info.add_string("<anon>"));
            let size = u32::try_from(struct_ty.size()).map_err(|_| {
                if let Some(declared_name) = struct_name.as_ref() {
                    Report::msg(format!(
                        ...
                        debug_info.get_string(*declared_name).unwrap()
                    ))
                } else {
                    Report::msg("invalid struct type: struct is too large")
                }
            })?;
            debug_info.add_type(DebugTypeInfo::Struct { name_idx, size, fields })
        },
        Type::Enum(enum_ty) => {
            let discrim_ty = register_debug_type(debug_info, None, None, enum_ty.discriminant())?;
            let name_idx = debug_info.add_string(enum_ty.name().clone());
            ...
            let variants = enum_ty
                ...
                .map(|((payload_offset, variant), discriminant)| {
                    let name_idx = debug_info.add_string(variant.name.clone());
                    let type_idx = variant
                        .value
                        .as_ref()
                        .map(|ty| register_debug_type(debug_info, None, None, ty))
                        .transpose()?;
            ...
            debug_info.add_type(DebugTypeInfo::Enum {
                name_idx,
                size,
        ...
        Type::Function(fty) => {
            let return_type_index = match fty.results() {
                [] => debug_info.add_type(DebugTypeInfo::Primitive(DebugPrimitiveType::Void)),
                [ty] => register_debug_type(debug_info, None, None, ty)?,
                types => {
                    ...
                    for (i, field) in ty.fields().iter().enumerate() {
                        let name_idx = debug_info.add_string(format!("{i}"));
                        let type_idx = register_debug_type(debug_info, None, None, &field.ty)?;
                        fields.push(DebugFieldInfo { name_idx, type_idx, offset: field.offset });
                    }
                    let name_idx = debug_info.add_string("<anon>");
                    debug_info.add_type(DebugTypeInfo::Struct { name_idx, size, fields })
                },
            };
            let mut param_type_indices = vec![];
            for param in fty.params() {
                param_type_indices.push(register_debug_type(debug_info, None, None, param)?);
            }
            debug_info.add_type(DebugTypeInfo::Function {
...
mod tests {
    ...
    fn registers_c_like_enum_debug_type() {
        let mut section = DebugInfoBuilder::<MastNodeRef, SourceNodeRef>::default();
        ...
        let DebugTypeInfo::Enum {
            ...
            variants,
        } = section.debug_info().get_type(type_idx).unwrap()
        else {
        ...
        assert_eq!(*size, 2);
        assert_eq!(
            section.debug_info().get_type(*discriminant_type_idx),
    ...
    fn registers_payload_enum_debug_type() {
        ...
        let DebugTypeInfo::Enum { variants, .. } = section.debug_info().get_type(type_idx).unwrap()
        else {
            panic!("expected enum debug type");
        };
        assert_eq!(variants.len(), 2);
        ...
        assert_eq!(
            section.debug_info().get_type(payload_type_idx),
    ...
    fn function_debug_types_preserve_resolved_struct_metadata() {
        ...
        let declared_name = section.add_string("take-account-id");
        let function_idx = register_debug_type(&mut section, Some(declared_name), None, &function)
            .expect("function type should register");

        let (return_type_idx, param_type_idx) =
            match section.debug_info().get_type(function_idx).unwrap() {
    ...
    fn assert_struct_debug_type(
        section: &DebugInfoBuilder<MastNodeRef, SourceNodeRef>,
        type_idx: DebugTypeIdx,
        expected_name: &str,
        expected_fields: &[(&str, &str)],
    ) {
        let DebugTypeInfo::Struct { name_idx, fields, .. } =
            section.debug_info().get_type(type_idx).unwrap()
        else {
        ...
        for (field, (expected_name, expected_type_name)) in fields.iter().zip(expected_fields) {
            ...
            let DebugTypeInfo::Struct { name_idx, .. } =
                section.debug_info().get_type(field.type_idx).unwrap()
            else {
